...
// from contracts.token.ERC20.IAFTERC20 import IAFTERC20
from contracts.token.ERC20.IExerciseSolutionToken import IESTERC20
...
func constructor{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    _wrap_token_address : felt
) {
    dummy_token_address_storage.write(_dummy_token_address);
    wrap_token_address_storage.write(_wrap_token_address);
...
@storage_var
func wrap_token_address_storage() -> (wrap_token_address_storage: felt) {
}
...
func get_tokens_from_contract{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (amount: Uint256) {
    ...
    // The "if then" logic above is not necessary, because when an account is not found on the storage var, the returned balance will be zero.
}
...
func withdraw_all_tokens{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (amount: Uint256) {
    ...
    let (wrap_token_amount : Uint256) = IERC20.balanceOf(wrap_token_address, caller);
    // It's necessary to add a burn functionality to the wrap token
    // Using burnFrom to avoid having to first transferFrom and then burn.
    let (burn_success : felt) = IESTERC20.burnFrom(wrap_token_address, caller, wrap_token_amount);
...
func deposit_tokens{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(amount: Uint256) -> (total_amount: Uint256) {
    ...
    let (success_wrap : felt) = IESTERC20.mint_tokens(wrap_token_address, amount); //In this case, the "caller" who will receive the wrap tokens is this_contract, and not to the caller. So we need to make an approve call followed by a transferFrom, so that the caller can receive the wrap token from this_contract.
    with_attr error_message("Wrap token minting failed."){
        assert success_wrap = 1;
    }
    let (success_approve : felt) = IESTERC20.approve(wrap_token_address, this_contract, amount);
    with_attr error_message("Approval failed"){
        assert success_approve = 1;
    }
    let (success_transfer : felt) = IESTERC20.transferFrom(wrap_token_address, this_contract, caller, amount);
