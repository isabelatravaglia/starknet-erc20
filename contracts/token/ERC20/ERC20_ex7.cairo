...
func constructor{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    recipient: felt,
    owner : felt, 
    _allowed_amount_lv1 : Uint256,
    _allowed_amount_lv2 : Uint256
) {
    ERC20.initializer(name, symbol, decimals);
    Ownable.initializer(owner);
    allowed_amount.write(1, _allowed_amount_lv1);
    allowed_amount.write(2, _allowed_amount_lv2);
...
@storage_var
func allowed_amount(level : felt) -> (withdraw_value : Uint256){
}
...
func get_tokens{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}() -> (amount: Uint256) {
    ...
    // let (amounts : Uint256) = allowed_amount.read();
    let (token_address : felt) = get_contract_address();
    let (level_granted) = allow_list.read(recipient);
    let zero_as_uint256 : Uint256 = Uint256(0, 0);

    if (level_granted == 1) {
        let (amount : Uint256) = allowed_amount.read(level_granted);
...
func request_allowlist_level{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(level_requested: felt) -> (level_granted: felt) {
    let (requester : felt) = get_caller_address();
    allow_list.write(requester, level_requested);
